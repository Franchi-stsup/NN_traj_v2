...
class BartNufftRecon(torch.autograd.Function):
    ...
    def backward(ctx, grad_output_image):
        ...
        try:
            ...
            # --- Reshape mrData ---
            shape =  list(grad_image_np.shape) + [1]*(11-len(grad_image_np.shape)-1) 
            # print(f"Shape {shape}")
...
class BartLoss(nn.Module):
    ...
    def shift_trajectory_torch(self, kx: torch.Tensor, ky: torch.Tensor):
        """
    ...
    def rotate_traj_torch(self, kx, ky, n_rotation=79):
        """
    ...
    def structural_similarity_index_torch(self, img1, img2):
        """
    ...
    def _torch_nufft_reconstruction2(self, mr_data, kspaceTrj):
        ...
        try:
            ...
            recon_img_scaled = self.rescale_recon_img_torch(recon_img, self.Nx, self.Ny, self.res)
    ...
    def compute_bart_loss_differentiable(self, predicted):
        ...
        for i in range(batch_size):
            try:
                ...
                kx_shifted, ky_shifted = self.shift_trajectory_torch(kx_single, ky_single)

                kspaceTrj = self.rotate_traj_torch(kx_shifted, ky_shifted)
                ...
                ssim_value = self.structural_similarity_index_torch(recon_img_scaled, ground_truth_resized)
